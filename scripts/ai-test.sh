...
# ── Section: models ───────────────────────────────────────────────────────────
run_models() {
    port_ready "$LITELLM_PORT" || { echo "✗ LiteLLM not running — run: make ai"; exit 1; }
    local _key="${LITELLM_MASTER_KEY:-}"
    ...
    while IFS= read -r model; do
        ...
        *)
        ...
        if [[ -n "$reply" ]]; then
            echo "  ✓ $model: $reply"; ((pass++)) || true
        else
            echo "  ✗ $model: no reply"; ((fail++)) || true
        fi
...
run_quality() {
    ...
    for model in $AI_QUALITY_MODELS; do
        ...
        if echo "$reply" | grep -q "4"; then
            ...
            echo "  ✗ $model: expected '4', got '$reply'"; ((fail++)) || true
        fi
    done
...
case "$SECTION" in
    ...
    models)  run_models ;;
    quality) run_quality ;;
    all)     run_infra; echo ""; run_models; echo ""; run_quality ;;
    *)       echo "Usage: ai-test.sh [infra|models|quality|all]"; exit 1 ;;
esac
